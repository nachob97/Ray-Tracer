...
class Sphere : public Object {
	...
	Sphere (Vect, double, Color*,double);
	...
	virtual double findIntersection(Ray* ray) {
		...
		if (discriminant > 0) {
			...
			if (root_1 > 0) {
				...
				return (root_1 > 0.00001 ? root_1 : -1);
			}
			else {
				// the second root is the smallest positive root
				double root_2 = ((sqrt(discriminant) - b)/2);
				return (root_2 > 0.00001 ? root_2 : -1);
			}
		}
		else {
...
Sphere::Sphere (Vect centerValue, double radiusValue, Color* colorValue,double c) {
	center = centerValue;
	radius = radiusValue;
	color = colorValue;
	coef = c;
